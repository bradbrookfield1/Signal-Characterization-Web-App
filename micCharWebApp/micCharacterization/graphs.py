...
def get_lag_autocorrelation(lib_list, hop_length):
    lag_autocorr_list = []
    if lib_list:
        for lib_entry in lib_list:
            onset_strength = librosa.onset.onset_strength(y=lib_entry[1], sr=lib_entry[0], hop_length=hop_length)
            tempogram = librosa.feature.tempogram(onset_envelope=onset_strength, sr=lib_entry[0], hop_length=hop_length)
            ac_global = librosa.autocorrelate(onset_strength, max_size=tempogram.shape[0])
            ac_global = librosa.util.normalize(ac_global)
            lag_seconds = np.linspace(0, tempogram.shape[0] * float(hop_length) / lib_entry[0], num=tempogram.shape[0])
            plt.figure(1, figsize=figsize).clf()
            ...
            plt.title('Autocorrelation (for first ' + str(hop_length) + ' samples)')
...
def get_bpm_autocorrelation(lib_list, hop_length):
    bpm_autocorr_list = []
    if lib_list:
        for lib_entry in lib_list:
            onset_strength = librosa.onset.onset_strength(y=lib_entry[1], sr=lib_entry[0], hop_length=hop_length)
            ...
            ac_global = librosa.autocorrelate(onset_strength, max_size=tempogram.shape[0])
            ac_global = librosa.util.normalize(ac_global)
            freqs = librosa.tempo_frequencies(tempogram.shape[0], hop_length=hop_length, sr=lib_entry[0])
            tempo = librosa.beat.tempo(onset_envelope=onset_strength, sr=lib_entry[0], hop_length=hop_length)[0]
            plt.figure(1, figsize=figsize).clf()
...
def get_onset_strength(lib_list, hop_length):
    onset_strength_list = []
    if lib_list:
        for lib_entry in lib_list:
            onset_strength = librosa.onset.onset_strength(y=lib_entry[1], sr=lib_entry[0], hop_length=hop_length)
            times = librosa.times_like(onset_strength, sr=lib_entry[0], hop_length=hop_length)
            plt.figure(1, figsize=figsize).clf()
...
def get_tempogram(lib_list, hop_length):
    tempo_chart_list = []
    if lib_list:
        for lib_entry in lib_list:
            onset_strength = librosa.onset.onset_strength(y=lib_entry[1], sr=lib_entry[0], hop_length=hop_length)
            ...
            tempo = librosa.beat.tempo(onset_envelope=onset_strength, sr=lib_entry[0], hop_length=hop_length)[0]
            plt.figure(1, figsize=figsize).clf()
            librosa.display.specshow(tempogram, sr=lib_entry[0], hop_length=hop_length, x_axis='time', y_axis='tempo', cmap='magma')
